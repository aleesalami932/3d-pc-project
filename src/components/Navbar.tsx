      ...
      <div className=" w-full flex justify-between items-center max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-white text-[18px] font-bold cursor-pointer flex">
            Alee&nbsp;<span className="sm:block hidden">| Learning 3D Web</span>
          </p>
        </Link>
        <ul className=" list-none hidden sm:flex flex-row gap-10">
          {navLinks.map((link) => (
            <li
              key={link.id}
              className={`hover:text-white text-[18px] font-medium cursor-pointer${active === link.title ? "text-white" : "text-secondary"}`}
              onClick={() => {
